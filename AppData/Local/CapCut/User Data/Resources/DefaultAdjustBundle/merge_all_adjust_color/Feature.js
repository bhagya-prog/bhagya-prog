...
class Feature
{
    ...
    onGetParameter(name)
    {
        switch(name) {
            case "is_skip_enable":
                return false;
            case "colorcorrection_alg_result": {
                    if (this.scenesMap['AmazingFeature_colorCorrection'] == null || this.scenesMap['AmazingFeature_colorCorrection'].visible == false)
                        return null;
                    
                    var tableCom = this.scenesMap['AmazingFeature_colorCorrection'].findEntityBy('EntityColorCorrection').getComponent('TableComponent').table;
                    if (tableCom.size() > 0) {
                        return tableCom
                    }
                    return null;
